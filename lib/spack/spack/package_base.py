...
#: Filename for Spack compile commands log
_spack_compile_logfile = "spack-cc.log"
...
class PackageBase(WindowsRPath, PackageViewMixin, metaclass=PackageMeta):
    ...
    @property
    def install_cps_prefix(self):
        return self.prefix.lib.cps

    @property
    def cps_file_path(self):
        return os.path.join(self.install_cps_prefix, self.name+".cps")

    def get_cps_prefix(self):
        return str(self.install_cps_prefix).replace(self.prefix, "@prefix@")
    ...
    def compile_log_path(self):
        """
        Return the log of compiler operations generated by the compiler wrapper
        """
        return os.path.join(self.stage.path, _spack_compile_logfile)

    @property
    def install_compile_log_path(self):
        """
        Return the install path for the compile log
        """
        return os.path.join(self.prefix, _spack_compile_logfile)
